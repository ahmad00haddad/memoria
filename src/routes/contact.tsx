...
function ContactPage() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSent(true);
      toast.success("تم إرسال رسالتك بنجاح! سنردّ عليكِ خلال 24 ساعة.");
    } catch (err: any) {
      console.error("[contact] failed to save message:", err);
      // Fallback: open mailto if DB fails
      const subject = encodeURIComponent(`[ميموريا] رسالة من ${name}`);
      const body = encodeURIComponent(`الاسم: ${name}\nالبريد: ${email}\nالهاتف: ${phone || "—"}\n\n${message}`);
      window.location.href = `mailto:ahmad000haddad@gmail.com?subject=${subject}&body=${body}`;
      toast.error("حدث خطأ — تم فتح بريدك كبديل.");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container-editorial py-16">
        ...
        <p className="text-muted-foreground mb-10 max-w-2xl">
          سواء كنتِ عروساً تبحثين عن مصوّرة، أو مصوّرة تريدين الانضمام، أو لديكِ ملاحظة على المنصّة — يسعدنا سماع منكِ.
        </p>

        <div className="grid gap-10 lg:grid-cols-2">
          <div className="space-y-6">
            <Item icon={Mail} label="البريد الإلكتروني">
              <a href="mailto:ahmad000haddad@gmail.com" className="hover:text-gold">
                ahmad000haddad@gmail.com
              </a>
            </Item>
            <Item icon={Clock} label="ساعات الرد">
              السبت – الخميس · 9 صباحاً – 8 مساءً (بتوقيت عمّان). نردّ عادة خلال 24 ساعة.
            </Item>
            <Item icon={MapPin} label="المقرّ">
          ...
          {sent ? (
            <div className="rounded-2xl border border-border/60 bg-card p-8 flex flex-col items-center justify-center text-center gap-4">
              <CheckCircle className="h-12 w-12 text-green-500" />
              <h2 className="font-serif text-2xl">تم إرسال رسالتك!</h2>
              <p className="text-muted-foreground text-sm">
                وصلت رسالتك بنجاح. سنتواصل معكِ على البريد الإلكتروني خلال 24 ساعة.
              </p>
              <button
            ...
            <form onSubmit={submit} className="rounded-2xl border border-border/60 bg-card p-6 space-y-4">
              ...
              <button
                disabled={submitting}
                type="submit"
                className="w-full rounded-md bg-primary text-primary-foreground py-2 text-sm hover:opacity-90 disabled:opacity-50 transition-opacity"
              >
                {submitting ? "جاري الإرسال..." : "إرسال الرسالة"}
              </button>
              <p className="text-xs text-muted-foreground">
                رسالتك ستصلنا مباشرة وستُحفظ بشكل آمن. لن يتم مشاركة بياناتك مع أي طرف آخر.
              </p>
            </form>
          )}
        </div>
...
function Item({ icon: Icon, label, children }: { icon: typeof Mail; label: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-4">
      <div className="shrink-0 grid h-10 w-10 place-items-center rounded-md bg-secondary">
